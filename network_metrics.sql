...
SELECT
    ...
    ROUND(SUM(engagements) / NULLIF(COUNT(*) FILTER (WHERE network = network), 0)::DECIMAL(9,2)) AS mean_engagement_per_post,
    ROUND(SUM(impressions) / NULLIF(COUNT(*) FILTER (WHERE network = network), 0)::DECIMAL(9,2)) AS mean_impression_per_post,
    ROUND(SUM(likes) / NULLIF(COUNT(*) FILTER (WHERE network = network), 0)::DECIMAL(9,2)) AS mean_likes_per_post,
    ROUND(SUM(comments) / NULLIF(COUNT(*) FILTER (WHERE network = network), 0)::DECIMAL(9,2)) AS mean_comments_per_post,
    ROUND(SUM(shares) / NULLIF(COUNT(*) FILTER (WHERE network = network), 0)::DECIMAL(9,2)) AS mean_shares_per_post,
...
\copy (Select * From network_metrics) To '/tmp/network_metrics.csv' With CSV DELIMITER ',' HEADER
